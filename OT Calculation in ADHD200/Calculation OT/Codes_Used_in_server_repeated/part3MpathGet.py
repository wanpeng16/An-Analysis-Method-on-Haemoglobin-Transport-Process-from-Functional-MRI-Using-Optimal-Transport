...
for i in range(len(subjects3)):
    for name in M1list:
        if name.startswith("stasMap%s"%(subjects4[i])):
            path = os.path.join(M1dir, name)
            M1path.append(path)

M2dir = "/home/guoxiaobo96/OT/MongeMap23/part4M23/"
M2list = os.listdir(M2dir)
M2path = []
M3path = []
for i in range(len(subjects3)):
    for name in M2list:
        if name.startswith("stasMap2%s"%(subjects4[i])):
            path = os.path.join(M2dir, name)
            M2path.append(path)
        if name.startswith("stasMap3%s"%(subjects4[i])):
            path = os.path.join(M2dir, name)
            M3path.append(path)

np.save("part4M1path", M1path)
np.save("part4M2path", M2path)
np.save("part4M3path", M3path)